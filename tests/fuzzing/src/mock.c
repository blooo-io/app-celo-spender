...
uint16_t get_public_key_string(bip32Path_t *bip32,
                               uint8_t *pubKey,
                               char *address,
                               uint8_t *chainCode,
                               uint64_t chainId) {
...
const uint8_t *parseBip32(const uint8_t *dataBuffer, uint8_t *dataLength, bip32Path_t *bip32) {
    if (*dataLength < 1) {
        PRINTF("Invalid data\n");
        return NULL;
    }

    bip32->len = *dataBuffer;

    dataBuffer++;
    (*dataLength)--;

    if (*dataLength < sizeof(uint32_t) * (bip32->len)) {
        PRINTF("Invalid data\n");
        return NULL;
    }

    if (bip32_path_read(dataBuffer, (size_t) dataLength, bip32->path, (size_t) bip32->len) ==
        false) {
        PRINTF("Invalid Path data\n");
        return NULL;
    }
    dataBuffer += bip32->len * sizeof(uint32_t);
    *dataLength -= bip32->len * sizeof(uint32_t);
