...
void handleProvideErc20TokenInformation(uint8_t p1,
    ...
    PRINTF("DEBUG: Token stored - Address: %02X%02X...%02X%02X, Ticker: %s, Decimals: %u, Chain ID: %u\n", 
           token->address[0], token->address[1], token->address[18], token->address[19],
           token->ticker, token->decimals, (uint32_t)token->chain_id);
    offset += 4;
    dataLength -= 4;
